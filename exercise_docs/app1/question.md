...
> [!IMPORTANT]
> ダミー IP アドレスが設定されているインタフェースでは、下記のように IP アドレスの削除と追加をすることで設定変更します。
...
> [!NOTE]
> `pingall` するとタイムアウト待ちで時間がかかるため、結果を載せておきます (問題 1 実施後・問題 2 設定前の状態)。移転したセグメントの Host.3a/b への通信が失敗していることがわかります。これが成功するように社内 NW のルータ・ファイアウォールを設定してください。
